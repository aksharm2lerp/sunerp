...
namespace Business.Interface
{
    public interface ISuperAdminService
    {
        ...
        #endregion
        #region "Item Master"
        PagedDataTable<ItemMaster> GetItemMasterAsync(int pid = 1, int pSize = 20, string search = null, string orderby = "ItemCode", string sortby = "ASC");

        Task<int> InsertOrUpdateItemMasterAsync(ItemMaster item);

        Task<ItemMaster> GetItemMasterAsync(int itemID);
        #endregion

        #region "Finish Good Item"
        Task<int> InsertOrUpdateFinishGoodItemAsync(ItemTableCollection item);
        PagedDataTable<ItemTableCollection> GetAllItemTableCollection(int pid = 1, int pSize = 20, string search = null, string orderby = "TableName", string sortby = "ASC");
        ItemTableCollection GetItemTableCollection(int id);

        Task<int> DeleteFinishGoodItem(int id, bool active);

        Task<int> DeleteFinishGoodItem1(FinishGoodItemDynamic finishGoodItemDynamic, int UserID);

        DataTable DynamicTableData(int ID);
        #endregion
    }
}
