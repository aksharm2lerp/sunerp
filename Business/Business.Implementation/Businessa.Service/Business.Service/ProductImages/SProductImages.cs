...
{
    public class SProductImages : IProductImages
    {
        private IConfiguration _config { get; set; }
        private string connection = string.Empty;
        public SProductImages(IConfiguration config)
        {
            _config = config;
        ...
        public async Task<ProductPhotoPath> IndexProductDisplay()
        {
            ProductPhotoPath result = new ProductPhotoPath();
            try
            {
        //        SqlParameter[] param = {
...
        //            new SqlParameter("@EmployeeID", employeeId)
        //};
                DataSet ds = await SqlHelper.ExecuteDatasetAsync(connection, CommandType.StoredProcedure, "Usp_GetAll_ProductImages");
                //if (ds != null)
                //{
                //    if (ds.Tables.Count > 0)
                //    {
...
                if (ds.Tables.Count > 0)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        List<ProductPhotoPath> listProductPhotoPath = new List<ProductPhotoPath>();

                        foreach (DataRow item in ds.Tables["Table"].Rows)
                        {
                            ProductPhotoPath productPhotoPath = new ProductPhotoPath();
                            productPhotoPath.ProductCategoryID = item["ProductCategoryID"].ToInt();
                            productPhotoPath.ProductImageID = item["ProductImageID"].ToInt();
                            productPhotoPath.ProductCategoryText = item["ProductCategoryText"].ToString();                            
                            productPhotoPath.ProductImageText = item["ProductImageText"].ToString();                            
                            productPhotoPath.ImagePath = item["ImagePath"].ToString();                            
                            productPhotoPath.Warehouse = item["Warehouse"].ToString();                            
                            productPhotoPath.IsActive = item["IsActive"].ToBoolDefaultTrue();                            
                            productPhotoPath.Description = item["Description"].ToString();
                            listProductPhotoPath.Add(productPhotoPath);
                        }
                        result.listItemDocument = listProductPhotoPath;


                        List<ProductImagesTxn> listProductImagesTxn = new List<ProductImagesTxn>();

                        foreach (DataRow item in ds.Tables["Table1"].Rows)
                        {
                            ProductImagesTxn productImagesTxn = new ProductImagesTxn();
                            productImagesTxn.ProductImageTxnID = item["ProductImageTxnID"].ToInt();
                            productImagesTxn.ProductImageID = item["ProductImageID"].ToInt();
                            productImagesTxn.ProductImageTxnText = item["ProductImageTxnText"].ToString();
                            productImagesTxn.ProductImageTxnPath = item["ProductImageTxnPath"].ToString();
                            productImagesTxn.IsActive = item["IsActive"].ToBoolDefaultTrue();
                            productImagesTxn.carouselProductImageTxnActive = item["carouselProductImageTxnActive"].ToString();
                            listProductImagesTxn.Add(productImagesTxn);
                        }
                        result.listProductImagesTxn = listProductImagesTxn;
                    }
                }
        ...
        public async Task<int> AddOrEditProductImages(ProductPhotoPath productPhotoPath, DataTable dataTable)
        {
            try
            {
                SqlParameter[] param = {
                        ...
                        ,new SqlParameter("@Warehouse",productPhotoPath.Warehouse)
                        ...
                        ,new SqlParameter("@ItemCategoryID",productPhotoPath.ProductCategoryID)
                        ,new SqlParameter("@ProductImagesTxn",dataTable)
                };
                param[8].Direction = ParameterDirection.Input;
                param[8].SqlDbType = SqlDbType.Structured;
                param[8].ParameterName = "@ProductImagesTxn";
                param[8].TypeName = "UDTT_ProductImagesTxn ";
