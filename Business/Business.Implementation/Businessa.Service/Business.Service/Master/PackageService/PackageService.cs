...
namespace Business.Service.Master.PackageService
{
    public class PackageService : IPackageService
    {
        ...
        public async Task<PagedDataTable<PackageFormTxn>> GetAllPackageForDropDownList()
        {
            DataTable table = new DataTable();
            int totalItemCount = 0;
            string packagename = null;
            ...
            try
            {
                using (DataSet ds = await SqlHelper.ExecuteDatasetAsync(connection, CommandType.StoredProcedure, "Usp_GetAll_PackageMasterDropdown"))
                {
                    if (ds.Tables.Count > 0)
                    {
                        table = ds.Tables[0];
                    }
                    lst = table.ToPagedDataTableList<PackageFormTxn>(0, 0, totalItemCount, packagename);
                    return lst;
                }
            }
            catch (Exception ex)
        ...
        public async Task<int> AddUpdateFormAssignToPackage(DataTable dataTable)
        {
            try
            {
                SqlParameter[] param = {
                new SqlParameter("@UDTVPackageFormTxn", dataTable)
                };
                param[0].Direction = ParameterDirection.Input;
                param[0].SqlDbType = SqlDbType.Structured;
                param[0].ParameterName = "@UDTVPackageFormTxn";
                param[0].TypeName = "UDTT_PackageFormTxn";
        ...
        public async Task<PagedDataTable<IconMaster>> GetAllIconNameForDropDownList()
        {
            DataTable table = new DataTable();
            int totalItemCount = 0;
            string packagename = null;
            PagedDataTable<IconMaster> lst = null;
            try
            {
                using (DataSet ds = await SqlHelper.ExecuteDatasetAsync(connection, CommandType.StoredProcedure, "Usp_GetAll_IconMasterForDropDown"))
                {
                    if (ds.Tables.Count > 0)
                    {
                        table = ds.Tables[0];
                    }
                    lst = table.ToPagedDataTableList<IconMaster>(0, 0, totalItemCount, packagename);
                    return lst;
                }
            }
            catch (Exception ex)
