...
namespace ERP.Controllers
{
    ...
    public class AccountController : SettingsController
    {
        ...
        private string tempSessionId = string.Empty;
        //private readonly IEmailService _emailService;
        public AccountController(IRoleAccessStore roleAccessStore, UserManager<UserMasterMetadata> userManager, SignInManager<UserMasterMetadata> signInManager, ISiteUserService siteUserService)
        {
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    tempSessionId = UserSessionID;
                    string userAgent = Request.Headers["User-Agent"].ToString();
                    string browserName = GetBrowserName(userAgent);



                    string LoginHis = await _siteUserService.LoginUserHistory(0, model.Email, DateTime.Now.ToString(), null, tempSessionId, model.Latitude, model.Longitude, ClientIP, browserName);
                    ...
                    else 
                    {
                        ...
                        string LogOutUser = await _siteUserService.LoginUserHistory(USERID, model.Email, null, DateTime.Now.ToString(), tempSessionId, "0", "0", null, null);
                        ClearCookies();
        ...
        #region Get browser name with version
        ...
        #endregion Get browser name with version
        [HttpGet]
        [AllowAnonymous]
        ...
        {
            await _roleAccessStore.RemoveRoleAccessAsync(USERID.ToString());
            //string LogOutUser = await _siteUserService.LoginUserHistory(USERID, null,null, DateTime.Now.ToString(), tempSessionId, "0", "0");
            //string LogOutUser = await _siteUserService.LoginUserHistory(USERID, null, null, DateTime.Now.ToString(),  UserSessionID , "0", "0", null, null);
            // Sign Out
            await _signInManager.SignOutAsync();
            // Clear all cookies
            ClearCookies();
