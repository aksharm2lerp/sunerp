    ...
    public class EmployeeController : SettingsController
    {
        ...
        public ActionResult Search(string keyword, int departmentID, int companyID)
        {
            try
            {
                ...
                PagedDataTable<EmployeeMaster> pds = _employeeService.GetAllEmployeesAsync(1, PAGESIZE, keyword, "", "ASC").Result;
                //if (model != null && model.DepartmentID > 0)
                //{
                //    pds = pds.Where(x => x.DepartmentID == model.DepartmentID).ToPagedDataTableList();
                //}
                foreach (var item in pds)
                {
                    item.EncryptedId = protector.Protect(item.EmployeeID.ToString());
        ...
        //        item.EncryptedId = protector.Protect(item.EmployeeID.ToString());
        //    }
        //    var server = new GridCoreServer<EmployeeMaster>(pds, query, false, "employeelistGrid", columns, PAGESIZE, pds.TotalItemCount, search)
        //        .Sortable()
        //        .Filterable(false)
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(id))
                {
                    ...
                    if (decryptedIntId > 0)
                    {
                        ...
                        if (!string.IsNullOrEmpty(addUpdateEmployee.ImagePath))
                            ViewData["Image"] = addUpdateEmployee.ImagePath;
                        ViewData["EmployeeID"] = addUpdateEmployee.EmployeeID;
                        if (!string.IsNullOrEmpty(addUpdateEmployee.FirstName) || !string.IsNullOrEmpty(addUpdateEmployee.MiddleName) || !string.IsNullOrEmpty(addUpdateEmployee.LastName))
                        {
                            ViewData["EmployeeName"] = string.Concat(addUpdateEmployee.FirstName, " ", addUpdateEmployee.MiddleName, " ", addUpdateEmployee.LastName);
                            ViewData["EmployeeCode"] = addUpdateEmployee.EmployeeCode;
                        }
                    }
                }
        ...
        public async Task<ActionResult> AddUpdateEmployeeSalaryBreakup(string employeeSalaryBreakupJson)
        {
            try
            {
                DataTable dataTable = new DataTable();
                List<EmployeeSalaryBreakupModel> employeeSalaryBreakupList = JsonConvert.DeserializeObject<List<EmployeeSalaryBreakupModel>>(employeeSalaryBreakupJson);
                ...
                foreach (var item in employeeSalaryBreakupList)
                {
                    ...
                    dataRow["EmployeeSalaryBreakupID"] = item.EmployeeSalaryBreakupID;
                    dataRow["EmployeeID"] = item.EmployeeID;
                    dataRow["SalaryHeadID"] = item.SalaryHeadID;
                    dataRow["SalaryHeadValue"] = item.SalaryHeadValue ?? 0;
                ...
                int salaryId = await _employeeService.AddUpdateEmployeeSalaryBreakup(dataTable);
                if (salaryId > 0)
                {
                    return Json(new { status = true, message = MessageHelper.Added });
                }
        ...
        public async Task<PartialViewResult> AddUpdateEmployeeMedicalHistory(int employeeMedicalDetailsID, int employeeId)
        {
            try
            {
                ...
                return PartialView("MedicalHistory/_addUpdateMedicalHistory", employeeMedicalHistory);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, ex.Message);
        ...
        public async Task<PartialViewResult> AddUpdateEmployeeMedicalInsurance(int employeeMedicalInsuranceId, int employeeId)
        {
            try
            {
                ...
                return PartialView("MedicalInsurance/_addUpdateMedicalInsurance", employeeMedicalInsurance);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, ex.Message);
        ...
        #region Employement Status Change by HR

        [HttpGet]
        public async Task<PartialViewResult> AddUpdateEmploymentStatusChangeHr(int employeeId)
        {
            try
            {
                EmploymentStatusChangeHr employmentStatusChangeHr = new EmploymentStatusChangeHr();
                employmentStatusChangeHr = _employeeService.EmploymentStatusChangeHr(employeeId).Result;
                return PartialView("_addUpdateEmploymentStatusChangeHr", employmentStatusChangeHr);
            }
            catch (Exception)
            {
        ...
        public async Task<ActionResult> AddUpdateEmploymentStatusChangeHr(EmploymentStatusChangeHr employmentStatusChangeHr)
        {
            try
            {
                employmentStatusChangeHr.CreatedOrModifiedBy = USERID;
                int adminId = await _employeeService.AddUpdateEmploymentStatusChangeHr(employmentStatusChangeHr);
                if (adminId > 0)
                {
                    return Json(new { status = true, message = MessageHelper.Added });
                }
...
        #endregion Employement Status Change by HR

        #region Employement Type Change by HR

        [HttpGet]
        public async Task<PartialViewResult> AddUpdateEmploymentTypeChangeHr(int employeeId)
        {
            try
            {
                EmploymentTypeChangeHr employmentTypeChangeHr = new EmploymentTypeChangeHr();
                employmentTypeChangeHr = _employeeService.EmploymentTypeChangeHr(employeeId).Result;
                return PartialView("_addUpdateEmploymentTypeChangeHr", employmentTypeChangeHr);
            }
            catch (Exception)
            {
        ...
        public async Task<ActionResult> AddUpdateEmploymentTypeChangeHr(EmploymentTypeChangeHr employmentTypeChangeHr)
        {
            try
            {
                employmentTypeChangeHr.CreatedOrModifiedBy = USERID;
                int adminId = await _employeeService.AddUpdateEmploymentTypeChangeHr(employmentTypeChangeHr);
                if (adminId > 0)
                {
                    return Json(new { status = true, message = MessageHelper.Added });
                }
...
        #endregion Employement Type Change by HR

        #region Employee Promotion & Increment

        [HttpPost]
        public ActionResult AddUpdateEmployeePromotionIncrement(EmployeePromotionIncrement employeePromotionIncrement)
        {
            try
            {
                employeePromotionIncrement.CreatedOrModifiedBy = USERID;

                if (employeePromotionIncrement.UpdatedCTC > 0)
                    employeePromotionIncrement.YearlyCTC = employeePromotionIncrement.UpdatedCTC * 12;

                int employeePromotionIncrementId = _employeeService.AddUpdateEmployeePromotionIncrementAsync(employeePromotionIncrement).Result;
                if (employeePromotionIncrementId > 0)
                {
                    return Json(new { status = true, message = MessageHelper.Added });
                }
                else
        ...
        #endregion Employee Promotion & Increment
    }
}